...
//velocidad unica/maxima de los motores
...
void actualizar_desplazamiento() {
    ...
    if (avance_bloqueado) estado_auto = FRENADO;

    switch (estado_auto){
        
        //si el estado es reposo, dejar todos los motores como free-roll
        case REPOSO:
            set_motor(M_DELANTERO_DER, M_LIBRE);
            set_motor(M_DELANTERO_IZQ, M_LIBRE);
            set_motor(M_TRASERO_DER, M_LIBRE);
            set_motor(M_TRASERO_IZQ, M_LIBRE);
            break;

        //si el estado es frenado se "bloquean" los motores    
        case FRENADO:
            set_motor(M_DELANTERO_DER, M_FRENADO_RAPIDO);
            set_motor(M_DELANTERO_IZQ, M_FRENADO_RAPIDO);
            set_motor(M_TRASERO_DER, M_FRENADO_RAPIDO);
            set_motor(M_TRASERO_IZQ, M_FRENADO_RAPIDO);
            break;
        ...
        case ATRAS:
            set_motor(M_DELANTERO_DER, M_MOV_ATRAS);
            set_motor(M_DELANTERO_IZQ, M_MOV_ATRAS);
            set_motor(M_TRASERO_DER, M_MOV_ATRAS);
            set_motor(M_TRASERO_IZQ, M_MOV_ATRAS);
            break;
...
void motores_config() {
	...
	CONN_MOTOR[M_TRASERO_IZQ][1] = GPIO1;
    CONN_MOTOR[M_TRASERO_IZQ][0] = LCD3;
    CONN_MOTOR[M_TRASERO_DER][1] = LCDRS;
    CONN_MOTOR[M_TRASERO_DER][0] = GPIO2;
    CONN_MOTOR[M_DELANTERO_IZQ][1] = LCD2;
    CONN_MOTOR[M_DELANTERO_IZQ][0] = LCD1;
...
void set_motor(motor_e m, estado_motor_e e){
